...
Description:
- Coarse-grained chromatin polymer (N beads)
- Includes:
    * Harmonic springs (backbone)
    * Soft repulsive interactions (non-bonded)
    * Active forces 
    * Hi-C based spring interactions (from contact map)

Output:
- XYZ trajectories (average and local sampling)
...
double Teff=8.0;
 ...
 //read contacts from prominent template 

FILE *fcontacts = NULL;    // for contacts

 fcontacts = fopen("./prom-contacts-1000/mc-1000-contact-file9.txt", "r");
   if (fcontacts== NULL) {
   ...
   int ch;
int Nc = 0;
while ((ch = fgetc(fcontacts)) != EOF) {
    if (ch == '\n') Nc++;
}
rewind(fcontacts);


   printf("The file number input9 has %d lines.", Nc);
   
rewind(fcontacts);  // now read from the start   

int *a = malloc(Nc * sizeof(int));
int *b = malloc(Nc * sizeof(int));
double *c = malloc(Nc * sizeof(double));
double *d = malloc(Nc * sizeof(double));
...
//Putting all the contact into arrays


for (i=0;i<Nc;i++)
fscanf(fcontacts,"%d \t %d \t %lf \t %lf \n", &a[i], &b[i], &c[i], &d[i]);
...
int num_groups = 0;
int groups[400][3]; 
double Factive_group[400] = {0}; // Store Factive for each group
double Fang_group[400] = {0};    // Store Fang for each group




int time_periods[400];  // Store time periods for each group
int current_event[400]; // Track the active event for each group
...
int last_update[400] = {0};  // track last update time for each group
...
if (rand_val < 0.5) {  // 50% chance
    ...
    Fang_group[i] = 0.0;
       ...
       fx=fy=fz=0;
        ...
        //Adding force due to soft-potential
...
for (j = 0; j < Nbeads; j++) {
    if (abs(bead - j) > 1) {  
        ...
        if (dr9 > 1e-12) {   /* protect against r ~ 0 */
            ...
            if (r < rm) {   /* APPLY soft force only for r < r_m */
                ...
                int s = abs(bead - j);   // contour separation
                ...
                if (s > 6) {
                    ...
                    V0_local = V0 * (1.0 + alpha * (s - 6.0) / (s0-6.0));
                }

                

                /* radial magnitude F(r) = V0*n1*n2 * u^{n2-1} * r^{n1-1} / rm^{n1} */
                double Fsoft = V0_local * eta1 * eta2 * u_pow * pow(r, eta1-2) / rm_eta1;
    ...
    if (neighbor != -1) {  // this bead is part of the contact
        dx1 = x[neighbor] - x[bead];
        dy1 = y[neighbor] - y[bead];
        dz1 = z[neighbor] - z[bead];

        double dr_hic = sqrt(dx1*dx1 + dy1*dy1 + dz1*dz1);
        
        if (dr_hic > 1e-12)
        {

        fx += K * (dr_hic - L) * (dx1 / dr_hic);
        fy += K * (dr_hic - L) * (dy1 / dr_hic);
        fz += K * (dr_hic - L) * (dz1 / dr_hic);
        }   
    }
...
//Sampling for average
 ...
 if (current_step % 10000 == 0 && current_step > 600000) {
    ...
    free(a); free(b); free(c); free(d);  // <- freeing contact arrays
